...
def encode(sm_list, device, pad_size=50):
    """
    Encodes list of smiles to tensor of tokens
    
    Args:
         sm_list (list) : List of SMILEs strings read from dataset
         device: The device on which to create output tensors: cpu or cuda
    ...
    for s in sm_list:
        ...
        res.append(tokens)
    tokens = torch.tensor(res, dtype=torch.long, device=device)
    smiles_lens = torch.tensor(lens, dtype=torch.long, device=device)
